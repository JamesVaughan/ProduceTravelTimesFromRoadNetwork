...
namespace ProduceTravelTimesFromRoadNetwork
{
    class Program
    {
        ...
        private static int GetStopCentroid(Network[] networks, int stopNode, int timeOfDay, Random r)
        {
            var zoneNumber = networks[0].GetZone(stopNode);
            networks[0].PickCentroidInZone(ref zoneNumber, r);
            return zoneNumber;
        }
            ...
            using (StreamWriter writer = new StreamWriter("ReadCellTraces.csv"))
            {
                ...
                foreach (var toWrite in Survey.EnumerateCellTraces(networkAM, @"G:\TMG\Research\Old\Montevideo\Cell data\dailytraces_caf.json\data_lake\Movilidad\Converted\Steps.csv", densityData)
                    ...
                    .Select(personRecord =>
                    {
                        ...
                        if (RecordsFor(buffer, buffer2, networks, personRecord, true, densityData))
                        {
